...
@testset "UMOCP.jl" begin
    ...
    obs_ind = 1
    obs_con1 = @constraint(n.ocp.mdl, [i=1:n.ocp.state.pts-1], 1 <= ((x[(i+1)]-timeSeq[i+1]*obs_info[obs_ind, 5]-obs_info[obs_ind, 1])^2)/((obs_info[obs_ind, 3]+ssm)^2) + ((y[(i+1)]-timeSeq[i+1]*obs_info[obs_ind, 6]-obs_info[obs_ind, 2])^2)/((obs_info[obs_ind, 4]+lsm)^2))
    newConstraint!(n,obs_con1,:obs_con1)
    
    
    ux_con2 = @constraint(n.ocp.mdl, [i=1:n.ocp.state.pts-1], (ux[i+1]-15)^2<=0.01)
    ...
    goal_con = @constraint(n.ocp.mdl, [i=n.ocp.state.pts-5:n.ocp.state.pts-1], ((psi[i]-pi/2)^2<=0.001))
    newConstraint!(n,goal_con,:goal_con)
    
    set_optimizer_attribute(n.ocp.mdl, "max_cpu_time", 10.0)
    set_optimizer_attribute(n.ocp.mdl, "max_iter", 1000)
